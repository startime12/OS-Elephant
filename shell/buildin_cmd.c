...
void buildin_ls(uint32_t argc,char** argv){
    ...
    while (arg_idx < argc){
        if (argv[arg_idx][0] == '-'){   // 如果是选项，单词的首字符是-
            if (!strcmp("-l", argv[arg_idx])){   // 如果是参数 -l
                long_info = true;
            }else if (!strcmp("-h", argv[arg_idx])){   // 如果是参数 -h
    ...
    if (pathname == NULL){   // 若只输入了ls或ls -l没有输入操作路径，则默认以当前路径作为操作路径
        if (NULL != getcwd(final_path, MAX_PATH_LEN)){
            pathname = final_path;
        }else {
    ...
    if (file_stat.st_filetype == FT_DIRECTORY){ // ls一个目录
        struct dir* dir = opendir(pathname);
        struct dir_entry* dir_e = NULL;
        char sub_pathname[MAX_PATH_LEN] = {0};
        uint32_t pathname_len = strlen(pathname);
        uint32_t last_char_idx = pathname_len - 1;
        memcpy(sub_pathname, pathname, pathname_len);   // 将需要ls的目录路径复制到sub_pathname
        if (sub_pathname[last_char_idx] != '/'){
            sub_pathname[pathname_len] = '/';
            ++pathname_len;
        }
        rewinddir(dir); // 把目录 dir 的指针 dir_pos 置 0
        if (long_info){ // 参数 -l
            char ftype;
            printf("total: %d\n", file_stat.st_size);
...
int32_t buildin_rmdir(uint32_t argc, char** argv){
    int32_t ret = -1;
    if (argc != 2){
        printf("rmdir: only suppory 1 argument!\n");
    }else {
        make_clear_abs_path(argv[1], final_path);
        /* 若删除的不是根目录 */
        if (strcmp("/", final_path)){
