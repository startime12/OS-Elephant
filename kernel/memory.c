...
void* sys_malloc(uint32_t size) {
    ...
    /* 若申请的内存小于等于1024，可在各种规格mem_block_desc中去适配 */
    }else {
        uint8_t descs_idx;
        for(descs_idx = 0; descs_idx < DESC_CNT; descs++){
            if(size <= descs[descs_idx].block_size) {
                // 从小往大后，找到后直接退出
                break;
            }
        }
        ...
        if(list_empty(&descs[descs_idx].free_list)) {
            // 分配1页框作为arena
            a = malloc_page(PF, 1);
            if(a == NULL) {
                lock_release(&mem_pool->lock);
            ...
            memset(a, 0, PG_SIZE);
            // 对于分配的小块内存，将desc置为相应内存块描述符，cnt置为此arena可用内存块数，large置为false
            a->desc = &descs[descs_idx];
            a->cnt = descs[descs_idx].blocks_per_arena;
            a->large = false;

            uint32_t block_idx;
            enum intr_status old_status = intr_disable();
            // 开始将arena拆分成内存块，并添加到内存块描述符的free_list中
            for(block_idx = 0; block_idx < descs[descs_idx].blocks_per_arena; block_idx++) {
        ...
        // 开始分配内存块
        b = elem2entry(struct mem_block, free_elem, list_pop(&(descs[descs_idx].free_list)));
        memset(b, 0, descs[descs_idx].block_size);
        a = block2arena(b);
        a->cnt--;
        lock_release(&mem_pool->lock);
        return (void*)b;
