...
static void partition_format(struct partition* part) {
    ...
    uint32_t inode_bitmap_sects =   // inode位图占用的扇区数 = 每个分区所支持最大创建的文件数 / 每扇区的位数
    DIV_ROUND_UP(MAX_FILES_PER_PART, BITS_PER_SECTOR);
    uint32_t inode_table_sects =    // inode结构占用的扇区数 = 每个分区所支持最大创建的文件数 / (每扇区字节大小 / inode结构大小)
    DIV_ROUND_UP(MAX_FILES_PER_PART * sizeof(struct inode), SECTOR_SIZE);
    uint32_t used_sects = boot_sector_sects + super_block_sects + inode_bitmap_sects + inode_table_sects;
    uint32_t free_sects = part->sec_cnt - used_sects;
    // 简单处理块位图占据的扇区数 其实就是还有free_sects块，应该分多少给块位图
    // 空闲块位图和空闲块数量相互依赖，相当于两个互相决定对方大小的变量
    // 例：先算出2*4096 + 1块空闲块需要3个空闲块位图，所以还剩(2*4096 + 1 - 3)块空闲块
    // (2*4096 + 1 - 3)块空闲块只需要2个空闲块位图，所以2*4096 + 1块空闲块其实也只需要2个空闲块位图
    ...
    // 按照缓冲区尺寸申请内存，申请的内存由内存管理清0后返回
    // 使用 uint8_t 指针可以明确地表示 buf 是指向字节级内存的指针
    ...
    // ①先将位图最后一字节到其所在扇区的结束全置为1，即超出实际块数部分直接占为己用
    ...
    // 由于inode_table中共4096个inode，位图inode_bitmap正好占用1扇区，即inode_bitmap+sects等于1
    // 所以位图中的位全部代表inode_table中的inode，无需再像block_bitmap那样单独处理最后一扇区的剩余部分，inode_bitmap所在扇区中没有多余的无效位
...
void filesys_init() {
    ...
    printk("   root_dir_lba:0x%x\n", cur_part->sb->data_start_lba);//0xA67
    printk("   inode_table_lba:0x%x\n", cur_part->sb->inode_table_lba);//0x807
}
...
int32_t sys_mkdir(const char* pathname) {
    ...
    if (!sync_dir_entry(parent_dir, &new_dir_entry, io_buf)) {
        // sync_dir_entry 中将block_bitmap通过bitmap_sync同步到硬盘
...
struct dir* sys_opendir(const char* name) {
    ...
    if (inode_no == -1) { // 如果找不到目录，提示不存在的路径
        ...
        if (searched_record.file_type == FT_REGULAR) {  //普通文件
            printk("%s is regular file!\n", name);
        } else if (searched_record.file_type == FT_DIRECTORY) { // 目录
