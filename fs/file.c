...
// 分配1个扇区, 返回其扇区地址(part->sb->data_start_lba + bit_idx)
int32_t block_bitmap_alloc(struct partition* part) {
    ...
    // 和 inode_bitmap_alloc 不同, 此处返回的不是位索引
    // 而是具体可用的扇区地址
...
int32_t file_open(uint32_t inode_no, uint8_t flag) {
    ...
    if(flag & O_WRONLY || flag & O_RDWR) {
        // 以下进入临界区前先关中断（判断->修改 两个步骤，需为原子性操作）
...
int32_t file_write(struct file* file, const void* buf, uint32_t count) {
    ...
    // 写入count个字节前，该文件已经占用的块数(包括第一次写刚刚分配的一个块)
    ...
    if(add_blocks == 0) {                           // 在同一扇区内写入数据，不涉及到分配新扇区
        ...
        }else {                                     // 未写入新数据之前已经占用了间接块，需要将间接块地址读进来
        ...
        /* 第三种情况:  新数据占据间接块 */
            ...
            while (block_idx < file_will_use_blocks){
                block_lba = block_bitmap_alloc(cur_part);
                if (block_lba == -1){
                    printk("file_wirte: block_bitmap_alloc for situation 3 failed\n");
                    return -1;
                }
                all_blocks[block_idx++] = block_lba;
                // 每分配一个块就将位图同步到硬盘
