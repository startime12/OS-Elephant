...
bool search_dir_entry(struct partition* part, struct dir* pdir, const char* name, struct dir_entry* dir_e) {
    ...
    // 写目录项的时候已保证目录项不跨扇区，这样读目录时容易处理，只申请容纳1个扇区的内存
    ...
    while (block_idx < block_cnt) {
        ...
        block_idx++;
        // 此时p_de已经指向扇区内最后一个完整目录项，需要恢复p_de指向为buf
...
void dir_close(struct dir* dir) {
     ...
     * 2 root_dir所在内存是低端1MB之内，并非在堆中，free会出问题 */
    if (dir == &root_dir) {
        // 不做任何处理直接返回
        return;
    }
...
bool sync_dir_entry(struct dir* parent_dir, struct dir_entry* p_de, void* io_buf) {
    ...
    // 感觉这里应该还有一步，把间接块存入 all_blocks
    // dir_inode->i_sectors[12]没有用到啊
...
// 读取目录, 成功返回 1 个目录项, 失败返回 NULL（借助dir结构的dir_pos游标）
struct dir_entry* dir_read(struct dir* dir) {
    ...
    while (block_idx < block_cnt) {
        ...
        if (all_blocks[block_idx] == 0) {   // 若此块地址为0，即空块，继续读出下一块
            block_idx++;
            continue;
        }
