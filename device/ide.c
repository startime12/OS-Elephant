...
static void select_disk(struct disk* hd) {
    ...
    // 将变量reg_device写入硬盘所在通道的device寄存器
...
static void select_sector(struct disk* hd, uint32_t lba, uint8_t sec_cnt) {
    ...
    // 在Sector count寄存器 写入要读写的扇区数（写入值为0时，表示256个扇区）
    outb(reg_sect_cnt(channel), sec_cnt);
    // 写入扇区号
    // 分别在LBA low LBA mid LBA high中写入LBA地址的低8、中8、高8位
    outb(reg_lba_l(channel), lba);
    outb(reg_lba_m(channel), lba >> 8);
    outb(reg_lba_h(channel), lba >> 16);
    // 在device寄存器中写入LBA地址的第24~27位
...
void ide_read(struct disk* hd, uint32_t lba, void* buf, uint32_t sec_cnt) {
    ...
    while (secs_done < sec_cnt) {
        // 读写端口0x1f2和0x172是8位寄存器，每次读写最多是256个扇区
...
void ide_init() {
    ...
    // 获取硬盘的数量 低端1MB以内虚拟地址和物理地址相同
    // 在物理地址0x475处存储着主机上安装的硬盘数量，它是由BIOS检测并写入的
    ...
    while(channel_no < channel_cnt) {
        ...
        case 1:
            ...
            channel->irq_no = 0x20 + 15;    // 从片8259a上倒数第一的中断引脚
            break;
        }
        // 未向硬盘写入指令时不期待硬盘的中断
        channel->expecting_intr = false;
        lock_init(&channel->lock);
        // 初始化为0，目的是向硬盘控制器请求数据后，硬盘驱动sema此信号量会阻塞线程
        // 直到硬盘完成后通过发中断，由中端处理程序将此信号量sema_up，唤醒线程
